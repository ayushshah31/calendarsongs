...
class _CustomCalendarState extends State<CustomCalendar> {
  ...
  Future<void> setAudioPlayer(int i) async{
    // _localPath = (await _findLocalPath("mantra"))!;
    setState(() {
      loading = true;
    });
    ...
    // print("Check pathintro: $introFile");
    // final checkPath = File(introFile);
    // print("filePathAudiointro: $checkPath");
    // print("Check pathmantra: $mantraFile");
    // final checkPath2 = File(mantraFile);
    // print("filePathAudiointro: $checkPath2");
    // _audioHandler = await AudioService.init(
    //   builder: () => AudioPlayerHandler(filePath: mantraFile),
    ...
    final playlist = ConcatenatingAudioSource(
      ...
      children: [AudioSource.file(introFile), AudioSource.file(mantraFile)],
    );
                      ...
                      child: Column(
                        ...
                        children: [
                          Container(
                            ...
                            // child: Image.asset('lib/assets/images/img$image.png',scale: 1,),
                            child: FutureBuilder(
                              future: _dataRequiredForBuild,
                              builder: (context,snapshot){
                                // print("snapshot: ${snapshot.data}");
                                // print("Loading : $loading");
                                // print("Down: $_downloading");
                                if(snapshot.hasData){
                                  // print("snapshotMD: ${snapshot.data!.mantraData[0]}");
                                  // print("snapshotM: ${snapshot.data!.mantra}");
                                  // print("snapshotT: ${snapshot.data!.tithiData["2023-11-11"]}");
                                  var res = getTithiDate(_selectedDay!);
                                    ...
                                    children: [
                                      Table(
                                        ...
                                        children: [
                                          ...
                                          //           stream: finalMantra.positionStream,
                                          //           builder: (context, snapshot) {
                                          //             // print("pos:${player.position.inMilliseconds.toDouble()}");
                                          //             // print("buf:${player.bufferedPosition.inMilliseconds.toDouble()}");
                                          //             return !loading? Row(
                                          //               children: [
                                          //                 Text("${finalMantra.position.inMinutes}:${finalMantra.position.inSeconds%60}"),
                                          //                 Expanded(
                                          //                   flex: 4,
                                          //                   child: Slider(
                                          //                       value: min(finalMantra.position.inMilliseconds.toDouble(),finalMantra.duration!.inMilliseconds.toDouble()),
                                          //                       min: 0.0,
                                          //                       divisions: finalMantra.duration!.inMilliseconds.toInt(),
                                          //                       max: finalMantra.duration!.inMilliseconds.toDouble()+5,
                                          //                       onChanged: (val){
                                          //                         // print("Val: $val");
                                          //                         // print("Player: ${player.duration!.inMilliseconds}");
                                          //                         // print(player.position);
                                          //                         // print(player.currentIndex);
                                          //                         // print(player.bufferedPosition);
                                          //                         // print(player.positionStream);
                                          //                         finalMantra.seek(Duration(milliseconds: val.toInt()));
                                          //                       }
                                          //                   ),
                                          //                 ),
                                          //                 Text("${finalMantra.duration!.inMinutes}:${finalMantra.duration!.inSeconds%60}"),
                                          //               ],
                                          ...
                                          //   children: [
                                          //     StreamBuilder<PlayerState>(
                                          //       stream: finalMantra.playerStateStream,
                                          //       builder: (context, snapshot) {
                                          //         return Text(
                                          //           mantraCounter<109?"Remaining: $mantraCounter":"Remaining: ∞"
                                          //         );
                                          //       }
                                          //     ),
                                          //     Row(
                                          ...
                                          //       mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                                          //       children: [
                                          //         IconButton(onPressed: ()=>finalMantra.seek(Duration(milliseconds: max(finalMantra.position.inMilliseconds - 5000,0))), icon: const Icon(Icons.replay_5,size: 35,)),
                                          //         StreamBuilder<PlayerState>(
                                          //           stream: finalMantra.playerStateStream,
                                          //           builder: (context, snapshot) {
                                          //             final playerState = snapshot.data;
                                          ...
                                          //                 icon: const Icon(Icons.play_arrow),
                                          //                 iconSize: 40.0,
                                          //                 onPressed: finalMantra.play,
                                          //               );
                                          //             } else if (processingState != ProcessingState.completed) {
                                          //               return IconButton(
                                          //                 icon: const Icon(Icons.pause),
                                          //                 iconSize: 40.0,
                                          //                 onPressed: finalMantra.pause,
                                          //               );
                                          //             } else if(mantraCounter>0){
                                          //               finalMantra.seek(Duration.zero);
                                          //               mantraCounter -= 1;
                                          ...
                                          //                 icon: const Icon(Icons.replay),
                                          //                 iconSize: 40.0,
                                          //                 onPressed: () => finalMantra.seek(Duration.zero),
                                          //               );
                                          //             }
                                          //           },
                                          //         ),
                                          //         IconButton(onPressed: ()=>finalMantra.seek(Duration(milliseconds: finalMantra.position.inMilliseconds + 5000)), icon: const Icon(Icons.forward_5,size: 35,))
                                          //       ],
                                          //     ),
                                          //   ]
                                          // )
